...
from keras.layers import Input, concatenate, ZeroPadding2D, add
...
def DenseNet(nb_dense_block=4, growth_rate=20, nb_filter=32, reduction=0.0, dropout_rate=0.0, weight_decay=0, classes=1000, weights_path=None):
    ...
    with tf.device('/gpu:0'):
	    ...
	    x = Conv2D(nb_filter*2, (7, 7) ,strides = (4,4),name='conv1',  kernel_initializer='he_normal', padding = 'same')(x)  ### Put strides of 2*2 here and padding = valid
	    x = BatchNormalization(axis=concat_axis, name='conv1_bn')(x)
	    x = Scale(axis=concat_axis, name='conv1_scale')(x)
	    x = Activation('relu', name='relu1')(x)
	    #x = ZeroPadding2D((1, 1), name='pool1_zeropadding')(x)
	    x = MaxPooling2D((3,3), strides=(2,2), name='pool1')(x) ### Put strides of 2*2 and pooling  3*3
...
def conv_block(x, stage, branch, nb_filter, dropout_rate=None, weight_decay=0):
    ...
    x1 = Conv2D(inter_channel, (1, 1), name=conv_name_base+'_x1',kernel_initializer='he_normal',activation = 'relu')(x)
    
    x2_1 = Conv2D(inter_channel/2, (1, 1), name=conv_name_base+'_x2_1',kernel_initializer='he_normal',activation = 'relu')(x)
    x2_2 = Conv2D(inter_channel, (3, 3), name=conv_name_base+'_x2_2',kernel_initializer='he_normal',activation = 'relu',padding = 'same')(x2_1)
    
    x3_1 = Conv2D(inter_channel/2, (1, 1), name=conv_name_base+'_x3_1',kernel_initializer='he_normal',activation = 'relu')(x)
    x3_2 = Conv2D(inter_channel, (3, 3), name=conv_name_base+'_x3_2',kernel_initializer='he_normal',activation = 'relu',padding = 'same')(x3_1)
    x3_3 = Conv2D(inter_channel, (3, 3), name=conv_name_base+'_x3_3',kernel_initializer='he_normal',activation = 'relu',padding = 'same')(x3_2)
    
    x4_1 = MaxPooling2D((2, 2),strides=(1,1),padding = 'same')(x)
    x4_2 = Conv2D(inter_channel, (1, 1), name=conv_name_base+'_x4_2',kernel_initializer='he_normal',activation = 'relu')(x4_1)
    ...
    x = Conv2D(nb_filter, (1, 1), name=conv_name_base+'_x2',kernel_initializer='he_normal',activation = 'relu')(x)
